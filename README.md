...
- 自然语言测试描述可以编译为包含 `navigate / click / fill / assert_*` 等步骤的结构化 `ExecutablePlan`。
...
仓库提供 Baseline 与 Context-Aware 编译策略的对照评测脚本，使用相同的中文 UI 用例统计 DSL 合法率、Locator 唯一命中率和端到端通过率。当前设定的评测目标是将 Locator 唯一匹配率从 68% 提升至 87%，端到端通过率从 54% 提升至 76%；这些数字是待验证目标，不是已经取得的实验结果。数据集、指标定义和运行方式见 [Context-Aware 评测说明](benchmarks/context-aware/README.md)。
